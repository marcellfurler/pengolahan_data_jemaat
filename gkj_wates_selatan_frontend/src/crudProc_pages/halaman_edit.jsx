...
const EditJemaat = () => {
  ...
  const [statusType, setStatusType] = useState(""); // ✅ untuk sertifikat
  ...
  const handleFileChange = (e, type) => {
    ...
    setStatusType(type); // simpan tipe sertifikat
  };
  ...
  const handleSubmit = async (e) => {
    ...
    for (const key in formData) {
      if (key !== "foto") formDataToSend.append(key, formData[key]);
    }

    // kirim file foto
    if (formData.foto instanceof File) formDataToSend.append("foto", formData.foto);

    // kirim file sertifikat + tipe
    if (selectedFile && statusType) {
      formDataToSend.append("sertifikat", selectedFile);
      formDataToSend.append("statusType", statusType); // backend pakai untuk folder
    }
    ...
    <>
      ...
      <div className="container my-5">
        <div className="card shadow border-0 mb-4" style={{ borderRadius: "12px" }}>
          <div className="card-header text-white d-flex align-items-center justify-content-center position-relative" style={{ backgroundColor: "#004d97", borderBottom: "none", height: "60px" }}>
            <button type="button" onClick={() => navigate(-1)} className="btn btn-light btn-sm fw-bold position-absolute start-0 ms-3" style={{ color: "#004d97" }}>
          ...
          <div className="card-body p-4 bg-light">
            <form onSubmit={handleSubmit} className="row g-3">
              ...
              <div className="col-md-6">
                ...
                <input type="text" name="golonganDarah" value={formData.golonganDarah} onChange={handleChange} className="form-control" />

                <label className="form-label fw-bold mt-3">Telepon</label>
                <input type="text" name="telepon" value={formData.telepon} onChange={handleChange} className="form-control" />

                <label className="form-label fw-bold mt-3">Alamat</label>
                <textarea name="alamat" value={formData.alamat} onChange={handleChange} className="form-control" rows="3"></textarea>
              ...
              <div className="col-12">
                ...
                <div className="row">
                  ...
                  <div className="col-md-4">
                    ...
                    <select name="statusBaptis" value={formData.statusBaptis} onChange={handleChange} className="form-select">
                      <option value="">Belum Baptis</option>
                      <option value="Baptis">Baptis</option>
                    </select>

                    {initialStatus.baptis !== "Baptis" && formData.statusBaptis === "Baptis" && (
                      <div className="mt-3">
                        <label className="form-label fw-bold">Upload Sertifikat Baptis</label>
                        <input type="file" accept="image/*,application/pdf" className="form-control" onChange={(e) => handleFileChange(e, "baptis")} />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="col-md-4">
                    ...
                    <select name="statusSidi" value={formData.statusSidi} onChange={handleChange} className="form-select">
                      <option value="">Belum Sidi</option>
                      <option value="Sidi">Sidi</option>
                    </select>

                    {initialStatus.sidi !== "Sidi" && formData.statusSidi === "Sidi" && (
                      <div className="mt-3">
                        <label className="form-label fw-bold">Upload Sertifikat Sidi</label>
                        <input type="file" accept="image/*,application/pdf" className="form-control" onChange={(e) => handleFileChange(e, "sidi")} />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="col-md-4">
                    ...
                    <select name="statusNikah" value={formData.statusNikah} onChange={handleChange} className="form-select">
                      <option value="">Belum Nikah</option>
                      <option value="Menikah">Menikah</option>
                    </select>

                    {initialStatus.nikah !== "Menikah" && formData.statusNikah === "Menikah" && (
                      <div className="mt-3">
                        <label className="form-label fw-bold">Upload Sertifikat Nikah</label>
                        <input type="file" accept="image/*,application/pdf" className="form-control" onChange={(e) => handleFileChange(e, "nikah")} />
                      </div>
                    )}
                  </div>
                </div>
